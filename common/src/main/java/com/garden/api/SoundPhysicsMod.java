...
import com.garden.api.config.SoundRateConfig;
import com.garden.api.config.OcclusionConfig;
import com.garden.api.config.ReflectivityConfig;
import com.garden.api.config.SoundPhysicsConfig;
...
public abstract class SoundPhysicsMod {
    ...
    public static final String MODID = "garden_api";
    ...
    private void initConfig() {
        if (CONFIG == null) {
            CONFIG = ConfigBuilder.builder(SoundPhysicsConfig::new).path(getConfigFolder().resolve(MODID).resolve("garden_api.properties")).build();
        }
    }
