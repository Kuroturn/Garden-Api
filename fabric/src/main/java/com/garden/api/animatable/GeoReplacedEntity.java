...
public interface GeoReplacedEntity extends SingletonGeoAnimatable {
	...
	default <D> void setAnimData(Entity relatedEntity, SerializableDataTicket<D> dataTicket, D data) {
		if (relatedEntity.level().isClientSide()) {
			...
			GardenApiNetwork.sendToTrackingEntityAndSelf(new EntityAnimDataSyncPacket<>(relatedEntity.getId(), dataTicket, data), relatedEntity);
		}
	}
	...
	default void triggerAnim(Entity relatedEntity, @Nullable String controllerName, String animName) {
		if (relatedEntity.level().isClientSide()) {
			...
			GardenApiNetwork.sendToTrackingEntityAndSelf(new EntityAnimTriggerPacket(relatedEntity.getId(), controllerName, animName), relatedEntity);
		}
	}
	...
	default void stopTriggeredAnimation(Entity relatedEntity, @Nullable String controllerName, @Nullable String animName) {
		if (relatedEntity.level().isClientSide()) {
			...
			GardenApiNetwork.sendToTrackingEntityAndSelf(new StopTriggeredEntityAnimPacket(relatedEntity.getId(), controllerName, animName), relatedEntity);
		}
	}
