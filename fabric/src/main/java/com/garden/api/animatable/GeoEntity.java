...
public interface GeoEntity extends GeoAnimatable {
	...
	default <D> void setAnimData(SerializableDataTicket<D> dataTicket, D data) {
		...
		if (entity.level().isClientSide()) {
			...
			GardenApiNetwork.sendToTrackingEntityAndSelf(new EntityAnimDataSyncPacket<>(entity.getId(), dataTicket, data), entity);
		}
	}
	...
	default void triggerAnim(@Nullable String controllerName, String animName) {
		...
		if (entity.level().isClientSide()) {
			...
			GardenApiNetwork.sendToTrackingEntityAndSelf(new EntityAnimTriggerPacket(entity.getId(), controllerName, animName), entity);
		}
	}
	...
	default void stopTriggeredAnimation(@Nullable String controllerName, @Nullable String animName) {
		...
		if (entity.level().isClientSide()) {
			...
			GardenApiNetwork.sendToTrackingEntityAndSelf(new StopTriggeredEntityAnimPacket(entity.getId(), controllerName, animName), entity);
		}
	}
