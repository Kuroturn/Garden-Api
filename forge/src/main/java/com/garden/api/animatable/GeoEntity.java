...
import com.garden.api.network.packet.EntityAnimDataSyncPacket;
import com.garden.api.network.packet.EntityAnimTriggerPacket;
import com.garden.api.network.packet.StopTriggeredEntityAnimPacket;
 ...
 * <b>NOTE:</b> This <u>cannot</u> be used for entities using the {@link com.garden.api.renderer.GeoReplacedEntityRenderer}
 * as you aren't extending {@code Entity}. Use {@link GeoReplacedEntity} instead.
 * @see <a href="https://github.com/bernie-g/garden_api/wiki/Entity-Animations">GardenApi Wiki - Entity Animations</a>
 */
public interface GeoEntity extends GeoAnimatable {
	...
	default <D> void setAnimData(SerializableDataTicket<D> dataTicket, D data) {
		...
		if (entity.level().isClientSide()) {
			...
			GardenApiNetwork.send(new EntityAnimDataSyncPacket<>(entity.getId(), dataTicket, data), PacketDistributor.TRACKING_ENTITY_AND_SELF.with(() -> entity));
		}
	}
	...
	default void triggerAnim(@Nullable String controllerName, String animName) {
		...
		if (entity.level().isClientSide()) {
			...
			GardenApiNetwork.send(new EntityAnimTriggerPacket<>(entity.getId(), controllerName, animName), PacketDistributor.TRACKING_ENTITY_AND_SELF.with(() -> entity));
		}
	}
	...
	default void stopTriggeredAnimation(@Nullable String controllerName, @Nullable String animName) {
		...
		if (entity.level().isClientSide()) {
			...
			GardenApiNetwork.send(new StopTriggeredEntityAnimPacket(entity.getId(), controllerName, animName), PacketDistributor.TRACKING_ENTITY_AND_SELF.with(() -> entity));
		}
	}
