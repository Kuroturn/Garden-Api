...
public interface GeoBlockEntity extends GeoAnimatable {
	...
	default <D> void setAnimData(SerializableDataTicket<D> dataTicket, D data) {
		...
		if (level.isClientSide()) {
			...
			GardenApiNetwork.send(new BlockEntityAnimDataSyncPacket<>(pos, dataTicket, data), PacketDistributor.TRACKING_CHUNK.with(() -> level.getChunkAt(pos)));
		}
	}
	...
	default void triggerAnim(@Nullable String controllerName, String animName) {
		...
		if (level.isClientSide()) {
			...
			GardenApiNetwork.send(new BlockEntityAnimTriggerPacket<>(pos, controllerName, animName), PacketDistributor.TRACKING_CHUNK.with(() -> level.getChunkAt(pos)));
		}
	}
	...
	default void stopTriggeredAnimation(@Nullable String controllerName, @Nullable String animName) {
		...
		if (level.isClientSide()) {
			...
			GardenApiNetwork.send(new StopTriggeredBlockEntityAnimPacket<>(pos, controllerName, animName), PacketDistributor.TRACKING_CHUNK.with(() -> level.getChunkAt(pos)));
		}
	}
